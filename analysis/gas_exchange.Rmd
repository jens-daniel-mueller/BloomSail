...
df %>% 
  ggplot(aes(date_time, pCO2_water))+
  geom_point(data=water, aes(date_time, pCO2_water, col="measured"))+
  geom_path()+
  geom_point()+
  geom_point(data = df, aes(date_time, pCO2_water, col="interpol"))
...
- Air sea CO~2~ flux F: mol m–2 yr–1 (= 365 * mol m–2 d–1)

- Coefficient for k calculation (eg 0.251 in W14): cm hr-1 (m s-1)-2 

W14_simplyfied: F = 7.7 * 10–4 * U2 * dpCO2



```{r gas_exchange_calc_daily}
...
# calculate flux F [mol m–2 yr–1]

df <- df %>% 
  mutate(flux = k_value*dCO2) 


df %>% 
  ggplot(aes(date_time, flux, col=k_para))+
  geom_line()+
  labs(y="F (mol m-2 yr-1)")+
  facet_wrap(~freq)

# scale flux to time interval

df <- df %>% 
  mutate(scale = if_else(freq == "daily", 365, 365*24*2)) %>% 
  mutate(flux = flux / scale) %>% 
  group_by(freq, k_para) %>% 
  arrange(date_time) %>% 
  mutate(flux_cum = cumsum(flux)) %>% 
  ungroup()
...
- Conversion of CO2 concentration from mol kg-1 to mol m-3 required?
