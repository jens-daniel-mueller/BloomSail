...
- BloomSail observations were restricted to those made in surface water and  
- SOOP Finnmaid + GETM hydrographical model
...
```{r calculate_TPD_cumulative_tm, fig.asp=1}
...
rm(PD, tm_profiles_ID_long_day, p_tem, p_CT_star)
...
```{r TPD_daily_tm_time_series, fig.asp=1}
...
p_iCT <- iCT %>%
  ggplot() +
  geom_hline(yintercept = 0) +
  geom_path(data = tm_NCP_cum, aes(date_time, CT_star_i_cum), col = "black") +
  geom_path(aes(date_time_ID, CT_star_i_cum, col = i_method, linetype = i_res)) +
