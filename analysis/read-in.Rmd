...
Originally, HydroC pCO~2~ data  were provided by KM Contros after applying a drift correction to the raw data, which was based on pre- and post-deployment calibration results. Those data are read-in here. However, later, the post-processing was repeated based on a cleaned data set.
...
# th_deployment <- th %>% 
#   group_by(deployment) %>% 
#   summarise(deployment_duration = max(date_time) - min(date_time)) %>% 
#   ungroup() %>% 
#   arrange(deployment_duration)
...
Discrete samples were collected with a Niskin bottle and analyzed for C[T] and A[T] at IOW CO~2~ lab.
...
tb <- read_csv(here::here("data/input/TinaV/Bottle/Tracegases", "BloomSail_bottle_CO2_all.csv"),
...
Discrete samples were collected with a Niskin bottle and analysed for phytoplankton composition and biomass at IOW CO~2~ lab.
...
tp <- read_csv(here::here("data/input/TinaV/Bottle/Phytoplankton",
