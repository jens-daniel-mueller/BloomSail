...
- assigning mean date_time value to all profiles belonging to one cruise
...
bind_cols(start = min(df$date_time), 
          end = max(df$date_time),
          AT = AT_mean,
          sal = sal_mean) %>% 
  write_csv(here::here("Data/_summarized_data_files", "BloomSail_fixed_values.csv"))
...
sd_profiles_long <- sd_profiles %>% 
  pivot_longer(4:7, names_to = "parameter", values_to = "sd")
  
mean_profiles_long <- mean_profiles %>% 
  pivot_longer(4:7, names_to = "parameter", values_to = "value")
  
mean_profiles_long <- inner_join(mean_profiles_long, sd_profiles_long)

rm(sd_profiles, sd_profiles_long)
...
all <- mean_profiles_long %>% 
  filter(parameter %in% c("CT", "tem")) %>% 
  rename(group = ID)
...
and grouped by ID 
[pdf here](https://github.com/jens-daniel-mueller/BloomSail/tree/master/output/Plots/CT_dynamics/profiles_by_ID.pdf){target="_blank"}. The later gives an idea of the differences between stations at one point in time.
...
df_long <- df %>%
  ...
  filter(dep < 25) %>% 
  pivot_longer(6:9, values_to = "value", names_to = "parameter")
...
mean_profiles_long %>%
  write_csv(here::here("Data/_merged_data_files", "BloomSail_CTD_HydroC_CT_cumulative_profiles.csv"))
...
CT_tem %>% 
  ...
  geom_point(aes(dtem, dCT, fill=ID), shape=21)+
  scale_fill_viridis_d()+
...
```{r plot_hovmoeller_daily, fig.cap="Hovmoeller plots of daily changes in C~T~ and temperature. Note: Daily changes are currently plotted against the day when they were observed compared to the previous transect, although plotting against the mean date would be more plausible."}
...
- Descriptive variables
  ...
  - date_time_ID_diff: mean date of cruise
  - station
...
- cumulative negative changes of CT
...
- cumulative changes of CT above 10 m
- cumulative positive changes of CT above 10 m
...
- refer to mean date between cruises for incremental and cumulative changes?
