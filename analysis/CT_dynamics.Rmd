...
ggplot()+
  ...
  geom_point(aes(herrvik_lon, herrvik_lat))+
  geom_text(aes(herrvik_lon, herrvik_lat, label = "Herrvik"), nudge_x = -0.05)+
  geom_point(aes(ostergarn_lon, ostergarn_lat))+
  geom_text(aes(ostergarn_lon, ostergarn_lat, label = "Flux tower"),
            nudge_x = -0.07,
            nudge_y = 0.01)+
...
ggsave(here::here("output/Plots/Figures_publication/article", "station_map.png"),
       width = 220, height = 150, dpi = 300, units = "mm")
...
ggsave(here::here("output/Plots/Figures_publication/article", "data_coverage.png"),
       width = 130, height = 65, dpi = 300, units = "mm")
...
tb_long_mean <- tb_long %>% 
  mutate(dep_grid = as.numeric(as.character(cut(dep,
                        breaks = seq(-2.5,30,5),
                        labels = seq(0,25, 5))))) %>% 
...
p_AT <- tb_long_mean %>% 
  filter(dep_grid < 25, var == "AT") %>% 
  ggplot(aes(value, dep_grid))+
  annotate("rect", xmin = AT_mean-AT_sd,
                xmax = AT_mean+AT_sd,
                ymin = -Inf,
                ymax = Inf,
            alpha = 0.3)+
  ...
  labs(x = expression(A[T]~(µmol~kg^{-1})),
       y = "Depth (m)")+
...
p_CT <- tb_long_mean %>% 
  filter(dep_grid < 25, var == "CT") %>% 
  ggplot(aes(value, dep_grid))+
  geom_path(aes(col=ID))+
  geom_point(aes(fill=ID), shape=21)+
  scale_y_reverse(sec.axis = dup_axis())+
  labs(x = expression(C[T]~(µmol~kg^{-1})),
       y = "Depth (m)")+
  scale_fill_viridis_d(guide = FALSE)+
  scale_color_viridis_d(guide = FALSE)+
...
ggsave(here::here("output/Plots/Figures_publication/appendix", "tb_profiles.pdf"),
       width = 180, height = 80, dpi = 300, units = "mm")

ggsave(here::here("output/Plots/Figures_publication/appendix", "tb_profiles.png"),
       width = 180, height = 80, dpi = 300, units = "mm")

rm(tb_long_mean, p_AT, p_CT, p_nCT, tb_fix, AT_sd)
...
ggsave(here::here("output/Plots/Figures_publication/article", "profiles_all.png"),
       width = 180, height = 80, dpi = 300, units = "mm")
...
p_nCT_ID_cum <- 
  ...
  scale_y_reverse(expand = c(0,0),
                  position = "right",
                  sec.axis = dup_axis())+
  labs(x = expression(nC[T]~cum.~changes~(µmol~kg^{-1})),
       y = "Depth (m)")+
  scale_color_viridis_d(labels = cruise_dates$date_ID)+
  scale_fill_viridis_d(labels = cruise_dates$date_ID)+
...
Predefined integration depth levels in metres are: `r fixed_depths`
...
NCP_flux_diff <- NCP_flux %>% 
  ...
  pivot_longer(cols = "observed":"flux", names_to = "var", values_to = "value_diff")
...
NCP_mix <- NCP_mix %>% 
  mutate(value_diff = (nCT_surface - nCT_ML) * 1e-3 * i_dep_lim * (i_dep_mix_lim - i_dep_lim) / i_dep_mix_lim,
         ID = "180815") %>% 
...
nCT_inventory_mean <- tm_profiles_ID_long %>% 
  filter(ID == "180806",
         var == "nCT",
         dep < i_dep_mix_lim) %>% 
  summarise(nCT_surface = sum(value)/i_dep_mix_lim) %>% 
  pull()
...
rm(NCP_mix_deep, NCP_mix_deep_diff, NCP_flux, NCP_flux_diff, date_180806)
...
ggsave(here::here("output/Plots/Figures_publication/article", "atm_water_timeseries.png"),
       width = 140, height = 270, dpi = 300, units = "mm")
