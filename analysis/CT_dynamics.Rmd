...
```{r data_coverage, fig.cap="Spatio-temporal data coverage, indicated as station visitm over time. ID (color) refers to the starting date of the cruise, except for P14, which was visited twice during each cruise."}
...
At stations P07 and P10 discrete samples for lab measurmentm of C~T~ and A~T~ were collected. Please note that - in contrast to the pCO~2~ profiles - samples were taken on June 16, but removed here for harmonization of resultm.
...
Important notes:
- Spatio-temporal variation of AT is small, which jusitfies conversion of pCO~2~ to C~T~ based on a fixed mean AT - On July 31 we see a drop in surface salinity, associated with a rise in A~T~, clearly pointing at exchange of water masses, presumably later 
...
tb_surface %>% 
  ...
  labs(x="Mean transect date", y="nCT")

```

Important notes:
- C~T~ drop and temporal patterns observed in the C~T~/A~T~ time series agrees well with those found in the C~T~ time series derived from pCO~2~ measurementm


## Mean alkalinity

In order to derive C~T~ from measured pCO~2~ profiles, the alkalinity mean + sd in the upper 20 m and both stations was calculated as:
...
Likewise, the mean salinity amountm to:
...
tm_profiles <- tm_profiles %>% 
  mutate(nCT = carb(24, var1=pCO2, var2=1720*1e-6,
...
```{r plot_nCT_profiles_all, fig.cap="Mean vertical profiles per cruise day across all stations."}
...
```{r plot_nCT_profiles_individual, fig.cap="Mean vertical profiles per cruise day across all stations plotted indivdually. Ribbons indicate the standard deviation observed across all profiles at each depth and transect.", fig.asp=1.5}
  ...
  for(i_station in unique(tm_profiles$station)){
    ...
    if (nrow(tm_profiles %>% filter(ID == i_ID, station == i_station)) > 0){
      ...
      p_pCO2 <- 
        ...
        geom_point(data = tm_profiles_highres %>% arrange(date_time) %>% filter(ID == i_ID, station == i_station),
...
for(i_ID in unique(tm_profiles$ID)){
  ...
  sub_tm_profiles_long %>% 
    ggplot()+
    geom_path(data = tm_profiles_long, aes(value, dep, group=interaction(station, ID)), col="grey")+
...
```{r inCT_depth_distribution_180723}
...
```{r inCT_tem_distribution_180723}
...
Data setm for atmospheric and seawater observations were merged and interpolated to a common time stamp.
...
The cumulative inCT time series obtained through integration across the upper 10m of the water column was used for further calculations of NCP.
...
To derive an integrated NCP estimated, the observed change in inCT must be corrected for the air-sea flux of CO2. inCT was determined for the upper 10m of the water column. The MLD was always shallower 10m, except for the last cruise day. Therefore:
...
During the last cruise, deeper mixing up to 17m water depth was observed, resulting in increased nCT values at 0-10 m and a decrease of nCT in 10-17m. The loss of nCT in 10-17m can be assumed to be entirely cause by mixing with low-nCT surface water. Some nCT loss is balanced through nCT input attributable to the air-sea flux. Therefore, the observed loss, corrected for 7m/MLD-share of the air-sea-flux, was added to the integrated nCT changes in 0-10m.
...
```{r plot_inCT_time_series, eval=FALSE}
