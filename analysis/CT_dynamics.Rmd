...
p_CT_surf <- ts_profiles_surface_long_ID %>%
  filter(var == "CT") %>% 
  ggplot(aes(x=date_time))+
  geom_ribbon(aes(ymin=mean-sd, ymax=mean+sd), alpha=0.2)+
  geom_path(aes(y=mean))+
...
```{r plot_flux_time_series}

p_pCO2_surf + p_tem_surf + p_CT_surf + p_pCO2_atm + p_wind + p_flux_daily + p_flux_cum +
  plot_layout(ncol = 1)
...
ggsave(here::here("output/Plots/Figures_publication/article", "atm_water_timeseries.pdf"),
       width = 120, height = 250, dpi = 300, units = "mm")
