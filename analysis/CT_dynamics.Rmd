...
date_labels <- ymd(unique(ts_profiles$ID))
...
p_tem <- 
  ts_profiles %>% 
  ggplot(aes(tem, dep, col=ID, group = interaction(ID, station)))+
  geom_path()+
  scale_y_reverse(expand = c(0,0), limits = c(25,0))+
  labs(x = "Temperature (\u00B0C)",
       y = "Depth (m)")+
  scale_color_viridis_d(labels = date_labels,
                        guide = FALSE)

p_pCO2 <- 
  ts_profiles %>% 
  ggplot(aes(pCO2, dep, col=ID, group = interaction(ID, station)))+
  geom_path()+
  scale_y_reverse(expand = c(0,0), limits = c(25,0))+
...
p_CT <- 
  ts_profiles %>% 
  ggplot(aes(CT, dep, col=ID, group = interaction(ID, station)))+
  geom_path()+
  scale_y_reverse(expand = c(0,0), limits = c(25,0))+
