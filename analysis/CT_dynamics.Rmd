...
ggplot()+
  ...
  geom_line(data = fm, aes(lon, lat, group=ID), col="grey")+
  #geom_line(data = fm %>% filter(Area == "BS"), aes(Lon, Lat, group=ID), col="red")+
...
rm(map, map_low_res, lat_hi, lat_lo, lon_hi, lon_lo, fm)
...
```{r pub_fig_CT_tem_hov_abs_profiles_cum, include=FALSE}
...
rm(p_CT_hov, bin_CT, p_tem_hov, bin_Tem, p_CT_ID_cum, p_tem_ID_cum)
...
iCT_grid_sign <- tm_profiles_ID_long %>% 
  select(ID, date_time_ID, date_time_ID_ref) %>% 
  unique() %>% 
  expand_grid(sign = c("pos", "neg"))

iCT_grid_total <- tm_profiles_ID_long %>% 
  select(ID, date_time_ID, date_time_ID_ref) %>% 
  unique() %>% 
...
iCT_sign_temp <- tm_profiles_ID_long %>% 
...
iCT_total_temp <- tm_profiles_ID_long %>% 
  filter(var == "CT", dep < dep_i) %>% 
...
rm(iCT, dep_i)
# iCT %>%
#   write_csv(here::here("Data/_merged_data_files", "iCT_dep_limitm.csv"))
...
tm_profiles_ID_mean_hydro <- tm_profiles %>% 
  select(-c(station,lat, lon, pCO2_corr, pCO2, CT, date_time)) %>% 
...
iCT_10_flux <- full_join(iCT_10, tm_og_flux) %>% 
  arrange(date_time)

rm(tm_og_flux, iCT_10, tm_og)


# linear interpolation of cumulative changes to frequency of the flux estimates estimates 
iCT_10_flux <- iCT_10_flux %>% 
  mutate(CT_i_cum = approxfun(date_time, CT_i_cum)(date_time),
...
iCT_10_mix <- tm_profiles_ID_long %>% 
  filter(ID == "180815",
         var == "CT",
         dep < 17,
         dep > 12) %>% 
...
```{r plot_iCT_time_series, eval=FALSE}
