...
The cumulative iC~T~ time series obtained through integration across the upper 10m of the water column was used for further calculations of NCP.
...
iCT_10 %>% 
  ...
  geom_hline(yintercept = 0)+
  geom_point(data = cruise_dates, aes(date_time_ID, 0), shape=21)+
  geom_line(aes(date_time_ID, CT_i_cum, col="iCT observed"))+
  scale_color_brewer(palette = "Set1", name="")+
  labs(y="iCT [mol/m2]", x="")
...
To derive an integrated NCP estimated, the observed change in iC~T~ must be corrected for the air-sea flux of CO~2~. iC~T~ was determined for the upper 10m of the water column. The MLD was always shallower 10m, except for the last cruise day. Therefore:

- Cumulative air-sea fluxes can be added completely to iC~T~ before Aug 7.
- Between Aug 7 and the last cruise on Aug 15 it was assumed, that the CO~2~ flux was homogenously mixed down to the MLD. The flux correction applied to the upper 10m can therefore be scaled with a factor 10m/MLD. 

```{r iCT_correction}

iCT_10_flux <- iCT_10 %>%
  select(date_time = date_time_ID, CT_i_cum)

date_180806 <- unique(iCT_10_flux$date_time)[7]
...
iCT_10_flux <- full_join(iCT_10_flux, ts_og_flux) %>% 
  arrange(date_time)
...
iCT_10_flux <- iCT_10_flux %>% 
  mutate(CT_i_cum = na.approx(CT_i_cum, rule = 2),
         ...
         CT_i_cum_total = CT_i_cum + flux_cum)
...
iCT_10_flux %>% 
  ...
  geom_line(aes(date_time, CT_i_cum_total, col="flux corrected"))+
  geom_ribbon(aes(date_time, ymin = CT_i_cum, ymax = CT_i_cum_total,
                  fill="Air-sea flux"), alpha=0.2)+
  scale_fill_brewer(palette = "Set1", name="")+
  scale_color_brewer(palette = "Set1", name="")+
  labs(y="integrated, cumulative CT [mol/m2]")+
  theme(axis.title.x = element_blank())
