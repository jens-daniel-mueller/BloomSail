...
- correction algorithm (Bittig vs. Fiedler, Miloshevich, Fietzek)
- smoothing
...
- Max depth for Down-Up-difference
...
A mean absolute residual threshold of >`r parameters$pCO2_resid_lim`% of final pCO~2~ was defined.

### Fitting algorithm


```{r fit_plot_all_response_times, include=TRUE}
...
The lowest standard deviation of tau values was found for a duration of:
...
cast_dep %>%
  ...
  labs(title = str_c("Date: ", parameters$example_ID, " | Station: ", parameters$example_station)) +
...
* RT: T-dependent response times applied (both independently quantified for 1W- and 8W-pump)
...
rm(i_ID, freq, RT_corr, tau_fit, tau_resid, tm_RT, tm)
  ...
  filter(ID == parameters$example_ID,
      ...
      tm_grid %>%
        ...
        labs(y="Depth [m]", x=expression(pCO[2]~(µatm)),
             title = str_c("Date: ",parameters$example_ID," | Station: ",parameters$example_ID))+
        facet_grid(tau_factor~., labeller = label_both)
...
tm_grid %>%
  ...
  geom_path(aes(up, dep_grid, col=correction, linetype ="up"))+
  scale_y_reverse()+
  scale_linetype(labels = c("upcast", "downcast"))+
  scale_color_brewer(palette = "Set1", direction = -1, labels = c("raw", "corrected"))+
  labs(y="Depth (m)", x=expression(pCO[2]~(µatm)))+
  theme(legend.title = element_blank(),
        legend.position = c(0.8,0.8),
        legend.background = element_rect(colour = "black"))

ggsave(here::here("output/Plots/Figures_publication/appendix", "pCO2_RT_corrected_profile_example.pdf"),
       width = 100, height = 140, dpi = 300, units = "mm")

ggsave(here::here("output/Plots/Figures_publication/appendix", "pCO2_RT_corrected_profile_example.png"),
       width = 100, height = 140, dpi = 300, units = "mm")
...
tm_grid %>%
  ...
  scale_color_discrete(name = "tau factor") +
  labs(
    x = expression(Delta ~ pCO[2] ~ ("%" ~ of ~ absolute ~ value)),
    y = "Depth [m]",
    title = str_c(
      "Date: ",
      parameters$example_ID,
...
tm_equi_delta %>%
  ...
  geom_hline(yintercept = 0) +
  geom_point() +
  labs(x = expression(Reference ~ pCO[2] ~ (µatm)),
       y = expression(Delta ~ pCO[2] ~ from ~ reference ~ (µatm)))
...
tm_grid_shallow_sum %>%
  filter(correction == "pCO2",
         ...
         tau_factor == parameters$tau_factor_used) %>%
  ggplot() +
  geom_path(aes(pCO2_delta, dep_grid)) +
  scale_y_reverse() +
  scale_color_discrete(name = "Tau factor") +
  labs(x = expression(Mean ~ "|" ~ Delta ~ pCO[2] ~ "|" ~ (µatm)), y = "Depth (m)") +
  xlim(0, NA)

ggsave(
  here::here(
    "output/Plots/Figures_publication/appendix",
    "pCO2_RT_corrected_profile_mean_abs_offset.pdf"
  ),
  width = 100,
  height = 140,
  dpi = 300,
...
ggsave(
  here::here(
    "output/Plots/Figures_publication/appendix",
    "pCO2_RT_corrected_profile_mean_abs_offset.png"
  ),
  width = 100,
  height = 140,
  dpi = 300,
...
df %>%
  ...
  labs(x = expression(pCO[2] ~ (µatm)),
       y = expression(Delta ~ pCO[2] ~ "/" ~ Delta ~ DIC ~ (µatm ~ µmol ^ {
         -1
       } ~ kg))) +
