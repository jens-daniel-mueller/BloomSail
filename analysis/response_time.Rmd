...
```{r all_response_times, eval=FALSE, echo=TRUE}
...
```{r thresholds_for_tau_and_fit_residual}
...
Estimated $\tau$ values were restricted to 
* mean absolute fit residual above `r resid_limit` % of the final equilibrium pCO~2~
* $\tau$ falling between `r tau_low` and `r tau_high` seconds
...
* $\tau$'s after removing fits with high absolute fit residual: `r n_tau_resid` (`r round(100*n_tau_resid/n_tau_total, 1)` %)
* $\tau$'s after restriction to $\tau$ thresholds: `r n_tau_resid_min_max` (`r round(100*n_tau_resid_min_max/n_tau_total, 1)` %)

`r rm(n_Zero_IDs, n_duration_intervals,n_tau_max, n_tau_total, n_tau_resid, n_tau_resid_min_max)`
...
```{r number_of_tau, fig.cap="Number of determined tau values as a function of the fit interval length, after applying a fit residual threshold"}
