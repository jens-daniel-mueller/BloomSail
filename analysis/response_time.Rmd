...
# subset equilibrated pCO2 levels as reference values

df_equi <- df %>% 
  filter(phase %in% c("mid")) %>% 
  group_by(ID, station, p_type, phase) %>% 
  top_n(5, row_number()) %>% 
...
```{r select_example_profiles, eval=FALSE}
...
First, we take a look at individual profiles.
...
equi_diff %>% 
 write_csv(here::here("data/_merged_data_files",
                      "BloomSail_CTD_HydroC_profiles_RT_reference-offset.csv"))
...
equi_diff_sub <- equi_diff %>%
  filter(ID == i_ID,
      ...
      RT_diff %>%
        ...
        geom_point(data = equi_diff_sub, aes(pCO2_equi, dep_int))+
...
i_station <- "P01"
      ...
      RT_diff %>%
        filter(ID == i_ID,
               ...
               correction == "pCO2_RT_mean") %>%
        arrange(dep_int) %>%
        ggplot(aes(d_pCO2_rel, dep_int, col=tau_factor))+
        geom_path()+
        geom_point()+
        scale_y_reverse(breaks=seq(0,40,2))+
...
equi_RT_diff <- full_join(RT_diff, equi_diff) %>% 
  filter(!is.na(pCO2_equi)) %>% 
  mutate(d_pCO2_equi = down - pCO2_equi)

```

```{r plot_pCO2_offset_reference_value, fig.cap="Offset between pCO~2~ downcast and upcast reference value. Panels highlight the effect of constant vs T-dependent tau estimates. Colour distinguish raw and corrected offsets."}

equi_RT_diff %>% 
  ggplot(aes(tau_factor, d_pCO2_equi, fill=correction))+
  geom_hline(yintercept = 0)+
  geom_violin()+
...
In order to decide, which conditions resulted in the best response correction, all profiles were pooled and some metrics derived. 

```{r calculate_performance_metrics}

 RT_diff_sum <- RT_diff %>% 
  #filter(p_type != "4") %>% 
  group_by(RT, tau_factor, dep_int, correction, p_type) %>% 
  ...
  ungroup() %>% 
  pivot_longer(cols = 6:9, names_to = "estimate", values_to = "dpCO2")


equi_RT_diff_sum <- equi_RT_diff %>% 
  group_by(correction, RT, tau_factor) %>% 
  summarise(d_pCO2_equi = mean(d_pCO2_equi, na.rm = TRUE),
            d_pCO2_equi_abs = mean(abs(d_pCO2_equi), na.rm = TRUE)) %>% 
  ungroup()
...
equi_RT_diff_sum <- equi_RT_diff_sum %>% 
  mutate(tau_factor = as.numeric(tau_factor))

equi_RT_diff_sum %>%
  ggplot(aes(tau_factor, d_pCO2_equi, col=RT, shape=correction, linetype=correction))+
  geom_hline(yintercept = 0)+
  geom_smooth(method = "lm")+
  geom_point()+
...
equi_RT_diff_sum %>%
  ggplot(aes(tau_factor, d_pCO2_equi_abs, col=RT, shape=correction, linetype=correction))+
  geom_hline(yintercept = 0)+
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), se=FALSE)+
  geom_point()+
  labs(y = "Mean delta pCO2 from reference [µatm]")

fit_factor_ref <- equi_RT_diff_sum %>% 
  filter(correction == "pCO2_RT_mean") %>% 
  group_by(RT) %>% 
  do(fit = lm(d_pCO2_equi ~ tau_factor, data = .)) %>% 
  tidy(fit) %>% 
  select(RT, term, estimate) %>% 
  spread(term, estimate)
...
RT_diff_sum %>% 
  ...
  geom_vline(xintercept = 0)+
  geom_vline(xintercept = c(-10,10), col="red")+
  facet_grid(estimate~RT*p_type)

```

```{r plot_mean_cast_delta_pCO2}
...
- Check why type 2, 3, and 4 profiles exceed 10 µatm absolute mean pCO2 deviation at depth > 10m
- Check whether irregular profiles can be excluded from response time optimization
