...
tm <- tm %>%
  select(date_time, ID, dep, tem, Flush, pCO2_corr, Zero_ID, duration, mixing)
...
i <- unique(tm_flush$Zero_ID)[30]

tm_flush_ID <- tm_flush %>%
...
A mean absolute residual threshold of >`r resid_limit`% of final pCO~2~ was defined.
...
for (i in unique(tm_flush$Zero_ID)) {
  for (max_duration in duration_intervals) {
    ...
    if (class(fit) == "nls"){
      ...
      pCO2_corr_start <- as.numeric(tidy(fit)[2,2])
      pCO2_corr_delta = pCO2_corr_end - pCO2_corr_start
      mean_abs_resid <- mean(abs(resid(fit))/pCO2_corr_end)*100
      
      temp <- as_tibble(bind_cols(Zero_ID = i,
                                  duration = max_duration,
                                  date_time = mean(tm_flush_ID$date_time),
                                  dep = mean(tm_flush_ID$dep),
                                  tem = mean(tm_flush_ID$tem),
                                  pCO2_corr = pCO2_corr_end,
                                  tau = tau,
                                  resid = mean_abs_resid))
      
      if (exists("tau_tm_flush")){tau_tm_flush <- bind_rows(tau_tm_flush, temp)}
        else {tau_tm_flush <- temp}
      
      if (mean_abs_resid > resid_limit){warn <- "orange"}
        else {warn <- "black"}
...
max_Zero_ID <- max(unique(tau_tm_flush[tau_tm_flush$date_time < ymd_hms("2018-07-17;13:08:34"),]$Zero_ID))

tau_tm_flush <- tau_tm_flush %>% 
  mutate(pump_power = if_else(Zero_ID <= max_Zero_ID, "1W", "8W"))



# subset determined tau values by residual threshold
tau_resid <- tau_tm_flush %>% 
  group_by(Zero_ID) %>% 
...
RT_fit %>% write_csv(here::here("data/_merged_data_files/response_time",
                                "tau_fit.csv"))

rm(list=setdiff(ls(), c("tau_resid", "RT_fit")))
...
tm <- tm %>%
  select(date_time, ID, type, station, dep, sal, tem,
         Zero, Flush, pCO2_corr, deployment, Zero_ID)
...
tm <- tm_corr
rm(RT_corr, i_ID, freq, tm_corr)

tm %>% 
 write_csv(here::here("data/_merged_data_files/response_time",
                      "tm_RT.csv"))

rm(tm, RT_fit, tau_resid)
...
tm_equi_pCO2_grid <- tm_equi_pCO2 %>%
  mutate(dep_grid = as.numeric(as.character(cut(dep, seq(0,40,1), seq(0.5,39.5,1))))) %>% 
...
tm_equi_pCO2_grid_sub <- tm_equi_pCO2_grid %>%
  filter(ID == i_ID,
         station == i_station)
      ...
      tm_grid %>%
        ...
        geom_point(data = tm_equi_pCO2_grid_sub, aes(pCO2_equi, dep_grid))+
...
rm(tm_equi_pCO2_grid_sub, i_ID, i_station)
...
tm_equi_delta <- full_join(tm_grid, tm_equi_pCO2_grid) %>% 
...
RT_fit <- read_csv(here::here("data/_merged_data_files/response_time",
                              "tau_fit.csv"))
...
- Calculate up- and downcast deviation for shallow (low pCO2) profiles only
