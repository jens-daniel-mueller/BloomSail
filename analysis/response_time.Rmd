...
RT_diff <- df %>% 
  ...
  group_by(ID, station, RT, tau_factor, dep_int, phase) %>%
...
A pdf with all absolute pCO~2~ offset profiles can be assessed [here](https://github.com/jens-daniel-mueller/BloomSail/tree/master/output/Plots/response_time/RT_correction_delta_pCO2_relative_profiles_discrete.pdf){target="_blank"}
...
equi_RT_diff_sum %>%
  ggplot(aes(tau_factor, d_pCO2_equi, col=RT, shape=correction))+
  geom_hline(yintercept = 0)+
  geom_point()


RT_diff_sum %>% 
  filter(correction == "pCO2_RT_mean", p_type == "1") %>% 
  ggplot()+
...
RT_diff_sum %>% 
  ...
  group_by(RT, estimate, p_type, correction) %>% 
...
RT_diff %>% 
  filter(dep_int < 20) %>% 
  ggplot(aes(tau_factor, d_pCO2, fill=correction))+
  geom_violin()+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 1)+
  facet_grid(RT~p_type)+
  theme_bw()

RT_diff_sum %>% 
  filter(dep_int < 20) %>% 
  group_by(RT, estimate, p_type, correction, tau_factor) %>% 
  summarise(mean_dpCO2 = mean(dpCO2)) %>% 
  ungroup() %>% 
...
# equilibrated reference pCO2 values
