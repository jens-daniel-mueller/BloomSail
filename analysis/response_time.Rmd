...
```{r identify_max_dep_pCO2_within_profiles_high_res}

tm_grid_stat_high_res <- tm_grid_high_res %>% 
  filter(correction == "pCO2_mean") %>% 
  #drop_na() %>% 
  group_by(ID, station) %>% 
  summarise(dep_max = max(dep_grid),
...
tm_grid_stat_high_res %>% 
  ggplot(aes(dep_max, pCO2_max))+
  geom_point()

tm_grid_high_res <- full_join(tm_grid_high_res, tm_grid_stat_high_res)
rm(tm_grid_stat_high_res)
...
ggplot()+
  geom_point(data = tm_grid_shallow_sum_mean %>% filter(estimate %in% c("mean_abs", "mean_rel_abs")),
