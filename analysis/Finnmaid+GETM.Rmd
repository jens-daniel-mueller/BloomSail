...
CT_tem <- full_join(CT_tem,
                    gt_2d_daily %>% select(date_time_ID = date_time, gt = SST))

```


```{r interpolate_gt_2d_to_fm, eval=FALSE}
...
```{r interpolate_fm_to_gt_2d_running_mean}
...
```{r FM_gt_2d_temp_timeseries_running_mean}
...
CT_tem_long %>% 
  ggplot(aes(date_time_ID, value, col=obs))+
  geom_hline(yintercept = 0)+
  geom_point()+
  geom_line()+
  scale_color_brewer(palette = "Set1")+
...
fm_gt_3d <- full_join(gt_3d %>% select(date_time_ID = date_time, dep, temp), 
                      CT_tem %>%  select(date_time_ID, dep, factor))


```

```{r interpolate_gt_to_fm}
