...
ts_th %>% 
  write_csv(here::here("Data/_merged_data_files/merging_interpolation", "ts_th_full.csv"))
...
th_comparison %>% 
  filter(!is.na(pCO2_corr)) %>% 
  ggplot() +
  geom_path(aes(date_time, pCO2_withoutAW-pCO2_withAW, col = "with - without AW"))+
  scale_color_brewer(palette = "Set1", name = "pCO2 record")+
  coord_cartesian(ylim = c(0,600))+
  labs(y=expression(pCO[2]~(µatm)), x="")+
  facet_wrap(~ID, scales = "free_x", ncol = 1)
